    ...
    ctx.on('llm/stream', (options, next) => {
        ...
        const wrappedStream = (async function* () {
            ...
            if (!hadFailure) {
                ...
                if (verbose)
                    console.log(`[agent-rate-limit] Recorded ${totalTokens} tokens${lastUsage ? ` (uncached: ${lastUsage.inputTokens}, cached: ${(lastUsage.cacheReadTokens ?? 0) + (lastUsage.cacheWriteTokens ?? 0)}, output: ${actualOutputTokens})` : ` (estimated: ${estimatedInputTokens}i)`}`);
