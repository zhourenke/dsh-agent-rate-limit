...
async function apply(ctx, config) {
		...
		const wrappedStream = (async function* () {
			...
			for await (const chunk of originalStream) {
				...
				if (chunk.type === "usage" && chunk.usage) {
					lastUsage = { inputTokens: chunk.usage.inputTokens, cacheReadTokens: chunk.usage.cacheReadTokens, cacheWriteTokens: chunk.usage.cacheWriteTokens };
					actualInputTokens = chunk.usage.inputTokens + (chunk.usage.cacheReadTokens ?? 0) + (chunk.usage.cacheWriteTokens ?? 0);
	...
	ctx.on("agent/request-error", async (payload, next) => {
		...
		// For non-retryable errors, delegate to the default handler
		return next();
	});
